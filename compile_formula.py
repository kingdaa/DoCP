...
def compile_formula(formula, verbose=False):
    ...
    body = ''.join(tokens)
    # Get words as string list in the formula
    words = filter(lambda s: s.isalpha(), re.split('([A-Z]+)', formula)[1:-1])
    non_zero_parms = set([word[0] for word in words])
    judge_stat = '==0 or '.join(non_zero_parms)
    judge_stat += '==0'
    judge_stat = 'False if (' + judge_stat + ') else'
    f = 'lambda %s: %s %s' % (parms, judge_stat, body)
    if verbose: print f
